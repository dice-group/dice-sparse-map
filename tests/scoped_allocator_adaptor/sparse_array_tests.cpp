...
void trailing_allocator_convention(Args...) {
    ...
    BOOST_REQUIRE((std::is_constructible<typename T::Array, Args..., Alloc const &>::value));
}
...
void trailing_allocator_convention_without_parameters() {
    ...
    // trailing_allocator thinks construction is not possible
    BOOST_REQUIRE((std::is_constructible<typename T::Array, Alloc const &>::value));
}
...
BOOST_AUTO_TEST_CASE(normal_compilation) {
    compilation<NORMAL<int>>();
}
BOOST_AUTO_TEST_CASE(normal_construction) {
    construction<NORMAL<int>>();
}
BOOST_AUTO_TEST_CASE(normal_set) {
    set<NORMAL<int>>({0, 1, 2, 3, 4});
}
BOOST_AUTO_TEST_CASE(normal_uses_allocator) {
    uses_allocator<NORMAL<int>>();
}
...
BOOST_AUTO_TEST_CASE(scoped_compilation) {
    compilation<SCOPED<int>>();
}
BOOST_AUTO_TEST_CASE(scoped_construction) {
    construction<SCOPED<int>>();
}
BOOST_AUTO_TEST_CASE(scoped_set) {
    set<SCOPED<int>>({0, 1, 2, 3, 4});
}
BOOST_AUTO_TEST_CASE(scoped_uses_allocator) {
    uses_allocator<SCOPED<int>>();
}
